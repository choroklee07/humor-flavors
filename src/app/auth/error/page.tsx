    ...
    <div className="flex min-h-screen items-center justify-center">
      <div className="w-full max-w-sm space-y-8 px-4 text-center">
        <div className="space-y-2">
          <p className="cyber-label tracking-[0.3em]">{`// AUTH FAILURE`}</p>
          ...
          <p className="cyber-label mt-4">AUTHENTICATION HANDSHAKE FAILED</p>
        </div>

        <div className="cyber-card cyber-corner rounded p-6 space-y-4">
          <p className="cyber-value text-xs text-left">
            {">"} OAUTH_EXCHANGE: FAILED<br />
            {">"} SESSION: NULL<br />
            {">"} ACTION: RETRY_REQUIRED
          </p>
          <Link href="/login" className="cyber-btn block w-full rounded px-6 py-3">
            RETRY LOGIN
          </Link>
        </div>
      </div>
